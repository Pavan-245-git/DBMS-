...
INSERT INTO Employee1(empno,emp_name,dept,salary,doj,branch) VALUES 
...
select empno,salary from Employee1;
select avg(salary) from Employee1;
select count(*) from Employee1;
select count(distinct emp_name) from Employee1;
select emp_name,sum(salary),count(*) from Employee1 group by(emp_name);
...
select * from Employee1 where emp_name='Amit' and salary>50000;
...
INSERT INTO EMPLOYEE2 (Emp_no, E_name, E_address, E_ph_no, Dept_no, Dept_name, Job_id, Designation, Salary)
VALUES 
...
SELECT * FROM EMPLOYEE2 WHERE Salary > ANY (SELECT Salary FROM EMPLOYEE2 WHERE Dept_name = 'IT PROFF');

ALTER TABLE EMPLOYEE2 ADD Joining_Date DATE;
SELECT * FROM EMPLOYEE2 WHERE YEAR(Joining_Date) > 1981 ORDER BY Designation ASC;
...
SELECT * FROM EMPLOYEE2 WHERE Designation IN ('CLERK','ANALYST');
